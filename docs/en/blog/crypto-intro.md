...
It's been cracked for 2000 years — 26 letters means only 25 offsets, so trying them all works (that's **brute force**). Try the [Caesar tool](/en/security) yourself.
...
Uses: password storage (databases keep only hashes — a leak reveals nothing), file integrity checks (was this download tampered with?), blockchains. Try the [SHA-256 tool](/en/security) and see how one changed character wrecks the hash.
...
- [CTF challenge](/en/challenge) levels 4 and 5 are Base64 and Caesar — you're armed now
