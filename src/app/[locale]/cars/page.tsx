...
import { getCarsContent } from "@/lib/cars";
import { CarSims } from "@/components/car-sims";

export async function generateMetadata({
  params,
}: PageProps<"/[locale]/cars">): Promise<Metadata> {
  const { locale } = await params;
  const validLocale = toLocale(locale) ?? routing.defaultLocale;
  const t = await getTranslations({ locale: validLocale, namespace: "Cars" });
  return { title: t("title"), description: t("description") };
}

export default async function CarsPage({
  params,
}: PageProps<"/[locale]/cars">) {
  ...
  const { intro, topics, terms } = getCarsContent(locale);
